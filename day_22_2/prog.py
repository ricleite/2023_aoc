...
# for each brick, check what other brick falls if removed
# somewhat slow - need to do the falling computations all over again - couple of mins

# a much faster alternative: an "expanding set" of falling bricks, by going through the supports
...
for i, brick in enumerate(bricks):
    ...
    update_fall(bricks_copy)

    count_bricks_that_fall = sum([1 for brick in bricks_copy if brick['start'][2] != bricks_by_id[brick['id']]['start'][2]])

    print(f"brick {i} - {count_bricks_that_fall}")
    count += count_bricks_that_fall
