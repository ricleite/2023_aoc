...
def kind(hand):
    ...
    # need to replace Js from hand into whatever makes it the strongest kind
    hand = [h for h in hand if h != 'J']
    hand = sorted(hand, key = lambda h: hand.count(h), reverse = True)

    while len(hand) < 5:
        hand.append(hand[0] if len(hand) > 0 else 'J') # handle "all jokers" edge case
